...
async def _cancel_and_await[T](task: asyncio.Task[T]) -> None:
    """Cancel a task and await its cleanup."""
    task.cancel()
    with contextlib.suppress(asyncio.CancelledError):
        await task


async def _await_with_time_limit[T](
    coro: Coroutine[Any, Any, T], limit: float | None
) -> T:
    ...
    if not math.isfinite(limit) or limit <= 0:
        with contextlib.suppress(AttributeError):
            coro.close()
        msg = "time_limit must be a positive finite number"
...
async def run_non_interactive(
    ...
    *,
    ...
    if not no_mcp and not effective_quiet:
        try:
